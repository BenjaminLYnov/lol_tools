...
namespace Collision
{
	...
	{
		int nbIteration = 0;
		sf::Vector2f dir = GetVectorMove(sprite1, sprite2);

		while (PixelPerfectTest(sprite1, sprite2))
		{
			if (nbIteration > 20)
				break;
			sprite1.move(dir);
	...
	// Cancel collision by a side
	void NotTriggerBySide(std::string side, sf::Sprite &sprite1, sf::Sprite &sprite2)
	{
		int nbIteration = 0;
		while (PixelPerfectTest(sprite1, sprite2))
		{
			...
			sf::Vector2f normalVec = GetVectorMove(sprite1, sprite2);

			if (side == "Left")
			{

				if (normalVec.x == -1 && normalVec.y == 0)
					sprite1.move(normalVec);
			}
			else if (side == "Right")
			{
			...
			else if (side == "Up")
			{
				...
				if (normalVec.x == 0 && normalVec.y == -1)
					sprite1.move(normalVec);
			}
			else if (side == "Down")
			{
			...
			else if (side == "Foot")
			{
				if (sprite1.getPosition().y < sprite2.getPosition().y)
					// if (sprite1.getPosition().y + (sprite1.getTextureRect().height * sprite1.getScale().y) / 2 < sprite2.getPosition().y)
					sprite1.move(0, -1);
				else
					break;
			}
	...
	// Check if there is collision by side
	bool OnCollisionBySide(std::string side, sf::Sprite &sprite1, sf::Sprite &sprite2)
	{
		if (PixelPerfectTest(sprite1, sprite2))
		{
			sf::Vector2f normalVec = GetVectorMove(sprite1, sprite2);
			...
			{
				if (normalVec.x == 0 && normalVec.y == -1 && sprite1.getPosition().y + (sprite1.getTextureRect().height * sprite1.getScale().y) / 2 < sprite2.getPosition().y)
					return true;
			}
		}
	...
	sf::Vector2f GetVectorMove(sf::Sprite &sprite1, sf::Sprite &sprite2)
	{
		// Get depth distance
		...
		// if (minDepthDistance >= depthDistanceUp || abs(depthDistanceRight - depthDistanceUp) < 10 || abs(depthDistanceLeft - depthDistanceUp) < 10)
		// {
		// 	minDepthDistance = depthDistanceUp;
		// 	normalVector = sf::Vector2f(0, -1);
		// }

		// if (minDepthDistance >= depthDistanceUp || abs(minDepthDistance - depthDistanceUp) < 20)
		// {
		// 	minDepthDistance = depthDistanceUp;
		// 	normalVector = sf::Vector2f(0, -1);
		// }
